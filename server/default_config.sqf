...
// Time settings
A3W_startHour = 8;                // In-game hour at mission start (0 to 23) - time is saved and restored between server restarts if A3W_timeSaving = 1
...
A3W_timeMultiplierNight = 14.0;     // Sets the speed of time between 8 PM and 5 AM
...
A3W_atmMaxBalance = 100000;       // Maximum amount of money that can be stored in a bank account (don't go over 16777216 as numbers start losing accuracy)
...
A3W_vehicleLifetime = 24;           // Maximum lifetime in hours for saved vehicles across server restarts, regardless of usage (0 = no time limit)
